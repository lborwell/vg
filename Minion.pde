abstract class Minion extends Creature {
  ...
  public Minion(int health, int range, int[] colour, int x, int y, float or, float xVel, float yVel, int atkDamage, int atkRange, int atkSpeed, Main m) {

    super(x, y, or, xVel, yVel, colour, atkDamage, atkRange, atkSpeed, m);
    this.atkRange = range;   
  ...
  public void removethis() {
    m.removeMinions.add(m.minions.indexOf(this));
  }

  public void kill() {
    state = 1;
    m.eno.attacking = false;
    m.eno.atkTarget = null;
  }
}
